...
def patch_outer_forward(model):
    ...
    # PEFT/Transformers gradient checkpointing calls these methods on the
    # outer conditional-generation model. Qwen3-ASR stores the actual token
    # embedding on `.thinker`, but the outer class does not delegate to it.
    # Patch the delegation here so this remains compatible with both the
    # repository model and future native Transformers implementations.
...
def mask_prefix_labels(full_inputs, prefix_inputs, pad_token_id=None):
    """Mask left- or right-padded prompt tokens while retaining target labels.

    Qwen's tokenizer defaults to left padding. Masking ``labels[:, :prefix_len]``
    therefore leaks the tail of the prompt into labels for shorter samples and
    masks part of the target for longer ones. Select the first *non-padding*
    positions instead, which works for either padding side.
    """
    labels = full_inputs["input_ids"].clone()
    full_attention = full_inputs["attention_mask"].bool()
    prefix_lengths = prefix_inputs["attention_mask"].sum(dim=1).tolist()
    for index, prefix_length in enumerate(prefix_lengths):
        valid_positions = torch.nonzero(full_attention[index], as_tuple=False).flatten()
        prefix_length = int(prefix_length)
        if prefix_length > valid_positions.numel():
            raise ValueError(
                f"prefix length {prefix_length} exceeds full input length "
                f"{valid_positions.numel()} for sample {index}"
            )
        labels[index, valid_positions[:prefix_length]] = -100
    labels[~full_attention] = -100
